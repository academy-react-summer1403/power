...
const SidebarNav: React.FC<SidebarNavProps> = ({ getLinkClassName }) => {
  return (
    <nav className="mt-4 w-1/2 lg:w-full flex flex-col gap-4">
      <Link title="داشبورد" to="dashboard" className={getLinkClassName("/StudentPanel/dashboard")}>
        <FaTachometerAlt className="mr-2" />
        <span className=" hidden lg:inline-block">داشبورد</span>
      </Link>
      <Link title="اطلاعات کاربری" to="user-info" className={getLinkClassName("/StudentPanel/user-info")}>
      ...
      <Link title="علاقه مندی ها" to="favorites" className={getLinkClassName("/StudentPanel/favorites")}>
        <FaHeart className="mr-2" />
        <span className=" hidden lg:inline-block">علاقه مندی ها</span>
      </Link>
      <Link to="settings" title="تنظیمات امنیتی" className={getLinkClassName("/StudentPanel/settings")}>
