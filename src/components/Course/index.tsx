    ...
    <>
      ...
      >
        <Image
          src={imageSrc}
          alt={"Pictures"}
          width={viewMode === "grid" ? 300 : 290}
          height={viewMode === "grid" ? 190 : 190}
          className={`rounded-xl ${
            viewMode === "grid" ? "h-[190px] w-[300px]" : "h-[190px] w-[290px]"
          } cover-image`}
        />
        <div
          className="absolute z-20 w-9 h-9 flex justify-center items-center cursor-pointer bg-white left-9 top-10 rounded-md dark:bg-[#2C2C2C]"
          onClick={AddCoursefavorite}
        >
          <Image
            src={localIsUserFavorite ? FavotiteTruePic : FavoritePic}
            alt="Pic"
          />
        </div>
        <div className="p-4 w-[95%] wrapper">
          ...
          <div className="w-full h-auto flex border-t border-[#E3E3F0] mt-6 justify-center lg:justify-between dark:border-[#444444]">
            ...
            <p className="text-[#5F5F66] dark:text-white flex items-center gap-1">
              <span className="text-[#5751E1] font-bold text-xl flex">
