...
export const CourseAddComment = () => {
  ...
  const onSubmit = async (values: { Describe: string; Title: string }) => {
    if (getItem("token")) {
      ...
      toast.error("برای ارسال نظر باید وارد شده باید.");
    }
  };
        ...
        {({ isSubmitting, setFieldValue, setFieldError, setFieldTouched }) => (
          <Form className="w-full h-auto flex flex-wrap items-center justify-center bg-[#F7F7FA] dark:bg-[#1F1F1F] rounded-xl mt-5 p-5">
            ...
            <div className="w-full max-w-[90%] mt-4">
              <h4 className="w-full text-[#6D6C80] dark:text-[#D1D1D1]">
                عنوان پیام
              </h4>
              <Field name="Title">
                {({ field }: any) => (
                  <InputEmoji
                    ...
                    onChange={(value) => {
                      handleInputChange(
                        value,
                        (sanitizedValue) =>
                          setFieldValue("Title", sanitizedValue),
                  ...
                  <InputEmoji
                    ...
                    onChange={(value) => {
                      handleInputChange(
                        value,
                        (sanitizedValue) =>
                          setFieldValue("Describe", sanitizedValue),
                        (errorMessage) =>
                          setFieldError("Describe", errorMessage)
                      );
                    }}
